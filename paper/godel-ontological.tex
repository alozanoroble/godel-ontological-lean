...
biconditionals; it makes God-like individuals unique, and its Theorem 3 needs
full $S5$. Fitting's makes positivity a predicate on \emph{extensions} rather
...
This note formalizes all five systems in Lean 4, in 371 lines of code with no
...
The biconditional in $\Gd^{\mathrm{A}}$ buys something Scott's system does not
have.
...
Theorem 3 (Thm.~\ref{thm:T3})         & all of Scott's          & symmetry \quad($B$) \\
Modal collapse (Thm.~\ref{thm:collapse}) & all of Scott's       & symmetry \quad($B$) \\
$r$ is equality (Thm.~\ref{thm:frame}) & all of Scott's         & symmetry \quad($B$) \\
One world (Thm.~\ref{thm:oneworld})   & all of Scott's          & universal \quad($S5U$) \\
\midrule
Anderson's Thm.~3 (Thm.~\ref{thm:andT3}) & all of Anderson's    & refl., trans., symm. \quad($S5$) \\
Fitting's Thm.~3 (Thm.~\ref{thm:fitT3})  & all of Fitting's     & none \quad($K$) \\
H\'ajek's Thm.~3 (Thm.~\ref{thm:hajT3})  & H:A12, A3 \emph{only} & symmetry \quad($B$) \\
\bottomrule
\end{tabular}
\end{center}

\noindent
By Proposition~\ref{prop:nosymm} the symmetry entries are sharp: rows three to
six all fail over a reflexive transitive frame, so no weakening in the $S4$
direction is available for Scott's system. The last three rows are the point of
Section~\ref{sec:variants}: Anderson pays more in frame strength than Scott
does, Fitting pays nothing at all, H\'ajek pays the same as Scott but with two
of his four axioms unused --- and all three avoid the collapse. Frame strength
...
The development is 1383 lines across seven files, of which 371 are code and the
