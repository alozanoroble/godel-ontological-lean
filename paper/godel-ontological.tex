...
All of this presupposes symmetry, and we show it cannot be dispensed with. An
...
biconditionals, which makes uniqueness especially direct --- though uniqueness
is available in Scott's system too, from A1b alone. Fitting's makes positivity
a predicate on \emph{extensions} rather
than intensional properties; its Theorem 3 needs no frame condition at all, and
the reason connects directly to the previous paragraph. H\'ajek's merges two
axioms into one weaker one, and the resulting definition of God-likeness makes
his Axioms 4 and 5 superfluous: Theorem 3 follows from two of his four axioms.
All three avoid modal collapse, and we exhibit two-world $S5$ models certifying
it.
...
\section{Symmetry is necessary}\label{sec:symm}
...
It cannot, and the reflexivity and transitivity of $S4$ are no substitute.
...
The biconditional in $\Gd^{\mathrm{A}}$ makes uniqueness especially direct.

\begin{proposition}[Monotheism]\label{prop:mono}
Over a reflexive frame, Anderson's axioms make God-like individuals unique: if
...
$\Gd^{\mathrm{A}}(y)$; and reflexivity gives $\varphi(y)$ at $w$, i.e. $y = x$.
\end{proof}
...
Over an $S5$ frame, Anderson's axioms entail $\Box\exists x\,\Gd^{\mathrm{A}}(x)$.
\end{theorem}

\noindent
Read the hypothesis as what \emph{this derivation} consumes, not as what
Anderson's system requires. Anderson himself (1990, fn.~5) notes that the
weaker logic $B$ --- that is, $KTB$ --- suffices for Theorem 3, and
Benzm\"uller, Weber and Woltzenlogel Paleo~\cite{bww2017} report it automated
in the weaker $KB$ again, and further report A4 and A5 \emph{redundant} for
this emendation, derivable from the rest. That last holds under the constant-
and varying-domain readings they analyse; under the \emph{mixed} reading
Anderson himself floats --- actualist quantifiers only in Theorem 3 and in the
definition of essence --- they find A4 still redundant, A5 independent, and a
countermodel to Theorem 3 itself. Domain choice is not a free parameter.

Our proof is therefore not minimal, in frame strength or in axioms; reaching
$KB$ would need a different route, since Lemma~\ref{lem:transfer} uses
transitivity and the necessary-existence step uses reflexivity. An earlier
version of this note reported the $S5$ as a cost of Anderson's variant, which
was a mistake of exactly the kind Section~\ref{sec:caveats} warns about.

\begin{proof}
God-likeness is an $\ess^{\mathrm{A}}$-essence of any God-like $x$: the forward
direction of the biconditional follows from A4 and reflexivity, the backward
direction from Lemma~\ref{lem:transfer}. The rest is as in
...
\begin{theorem}\label{thm:fitT3}
Fitting's axioms entail $\Box\exists x\,\Gd^{\mathrm{F}}(x)$, \emph{over an
arbitrary frame} --- the argument runs in $K$.
\end{theorem}

\begin{proof}
Theorem 1 goes through as before, giving a world $v$ accessible from $w$ and an
individual $g$, existing at $v$, with $\Gd^{\mathrm{F}}(g)$ as evaluated at
$w$. God-likeness is an essence of $g$, by the argument of
Theorem~\ref{thm:T2} with extensions in place of properties; A5 then gives
$\Box\exists z\,\Gd^{\mathrm{F}}(z)$ at $w$.
\end{proof}
...
corresponding steps --- Proposition~\ref{prop:mono}, Lemma~\ref{lem:transfer},
the essence theorem and the necessary-existence step --- are all
\emph{constructive}; Lean reports them as depending on no axioms at all. Only
Theorem~1, and hence Theorem~\ref{thm:andT3}, remains classical. Fitting keeps
A1b and with it the classical step.
\end{remark}
...
Theorem 3 (Thm.~\ref{thm:T3})         & all of Scott's          & symmetry \quad($KB$) \\
Modal collapse (Thm.~\ref{thm:collapse}) & all of Scott's       & symmetry \quad($KB$) \\
$r$ is equality (Thm.~\ref{thm:frame}) & all of Scott's         & symmetry \quad($KB$) \\
One world (Thm.~\ref{thm:oneworld})   & all of Scott's          & universal \quad($S5U$) \\
\midrule
Anderson's Thm.~3 (Thm.~\ref{thm:andT3}) & all of Anderson's$^{\dagger}$ & symmetry \quad($KB$)$^{\dagger}$ \\
Fitting's Thm.~3 (Thm.~\ref{thm:fitT3})  & all of Fitting's     & none \quad($K$) \\
...
\noindent
$^{\dagger}$ These are the published values, not ours. Our Lean derivation of
Anderson's Theorem 3 uses reflexivity, transitivity and symmetry and the whole
axiom set; Anderson (fn.~5) and~\cite{bww2017} give $KB$, with A4 and A5
redundant. Every other entry is what our proof consumes.
...
the same as Scott but with two of his four axioms unused, and Anderson --- on
the published analysis --- pays no more than Scott either. All three avoid the
collapse, at differing frame strengths. Frame strength is not the variable that
matters; the shape of the essence clause is.
...
\verb|scott_T3|                   & Theorem~\ref{thm:T3} \\
