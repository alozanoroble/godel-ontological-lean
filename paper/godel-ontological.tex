...
\paragraph{The warning.} Whether a property may mention a particular world is
a question about the \emph{comprehension principle} of the higher-order
language --- about what counts as a property --- and not about Scott's axioms.
Our shallow embedding makes every function $I \to W \to \Prp$ a property, so
$\varphi$ is available. The published dependency analyses reach Theorem 3
through Theorem 2 and accordingly list A4. We have not found the reduction in
the literature, and we make \emph{no claim of novelty}: the far likelier
explanation is that the divergence lives in Lemma~\ref{lem:haecceity} and that
the reduction is an artefact of the encoding rather than a fact about Scott's
system. It is reported because a formalization that silently admitted a
shortcut it did not mention would be the worse for it, and because knowing
where an encoding is more permissive than its source is exactly the sort of
thing formalization is for.
...
\noindent
$^{\ast}$ These rows are starred because they depend on
Lemma~\ref{lem:haecceity}, whose status is discussed there: the reduction is a
theorem of our encoding, but it may not be one about Scott's system as
intended, and the published dependency analyses give the unstarred route.
Where the two differ, trust the unstarred one for claims about Scott.
